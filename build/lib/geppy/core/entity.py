...
class KExpression(list):
    ...
    def simple_string(self):
        return ', '.join(ele.get_name() for ele in self)

    def __repr__(self):
        return str(self.__class__) + '[{}]'.format(', '.join(repr(p) for p in self))

    def str_repr(self, function_map={}):
        def decode_recursive(expression):
            symbol = expression.pop(0)
            if symbol.arity == 2:
                return symbol.str_repr(function_map).format(decode_recursive(expression), decode_recursive(expression))
            elif symbol.arity == 1:
                return symbol.str_repr(function_map).format(decode_recursive(expression))
            else:
                return symbol.name

        return decode_recursive(self.copy())
...
class Gene(list):
    ...
    def set_augmented(self, augmented):
...
class GeneLM(Gene):
    def __init__(self, state, pset, head_length, gene_memory, embedding, **kwargs):
        super().__init__(pset, head_length)
        self._head_length = head_length
        if kwargs.get('llm', False):
            genome = gene_memory.sample(embedding=embedding, state=state, **kwargs)
        else:
            genome = generate_genome(pset, head_length)
        list.__init__(self, genome)
...
class Chromosome(list):
    ...
    def __init__(self, gene_gen, n_genes, linker=None, prop_threshold=0.3, **kwargs):
        """
        ...
        self._pos_linker = linker
        self._linker = [self._pos_linker[random.randint(0, max(0, len(linker) - 1))] for _ in range(n_genes - 1)]
        if kwargs.get('conditional', False):
            if prop_threshold > random.random():
                temp = []
                state = []
                self._linker = [self._function_map[elem.name] for elem in gene_gen([[], []], llm=True, gene_count=n_genes, **kwargs)]
                for _ in range(n_genes):
                    if len(temp) == 0:
                        temp.append(gene_gen([self._linker, []], gene_count=1,llm=True, **kwargs))
                    else:
                        temp.append(gene_gen([self._linker, state], gene_count=1,llm=True, **kwargs))
                    state.append(Chromosome.calc_active_len(temp[-1]))
                list.__init__(self, (elem for elem in temp))
            else:
    ...
    def get_linker_as_gep_functions(self):
        if isinstance(self._linker, list):
            return [Function(elem.__name__, 2) for elem in self._linker]
        else:
            return Function(self._linker.__name__, 2)

    @classmethod
    def calc_active_len(cls, state):
        temp_arity = 0
        new_state = []
        for elem in state:
            if temp_arity == 0:
                temp_arity = elem.arity
            else:
                temp_arity += elem.arity - 1
    ...
    def print_raw_pheno(self):
        all_genes = [[indi.name for indi in Chromosome.calc_active_len(elem)] for elem in self]
        linker = [self.get_linker_as_gep_functions()] if not isinstance(self._linker, list) \
            else self.get_linker_as_gep_functions()
        linker = [elem.name for elem in linker]
        return [[all_genes, linker]]

    def print_raw_pheno_as_string(self, symbol_table=None):
        sequence = self.print_raw_pheno()[0]
        genes = sequence[0]
        linker = sequence[1]
        gene_str = [','.join(elem) for elem in genes]
        gene_str = ';'.join(gene_str)
        linker_str = ','.join(linker)

        ret_val = linker_str + ';' + gene_str
