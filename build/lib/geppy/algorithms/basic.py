...
def gep_simple(population, toolbox, n_generations=100, n_elites=1,
               stats=None, hall_of_fame=None, verbose=__debug__, log_genes=False,**kwargs):
    """
    ...
    :param n_elites: number of elites to be cloned to next generation - controls the proportion of the rebuild indis
    ...
    for gen in tqdm(range(n_generations + 1)):
        ...
        for ind, fit in zip(invalid_individuals, fitnesses):
            ind.uuid = str(uuid.uuid4())
            fitness_log[ind.uuid] = fit
            ind.fitness.values = fit
            glob_fit.append(fit[0])

        if kwargs.get('train_mem', False):
            #
            if gen%kwargs.get('epochs_wait',100)==0 and gen>1:
                try:
                    val = np.nanmin(glob_fit)*1.2
                    toolbox.train_memory(population=population, threshold=val, gen_=gen)
                except:
                    print(glob_fit)
        ...
        try:
            toolbox.log_per_epoch(logbook, hall_of_fame[0].uuid, gen)
        except Exception as error:
            print("Something went wrong with the callback")
        ...
        if log_genes:
            try:
                toolbox.write_pop(population, gen)
            except Exception as err:
                print(f"Issue for writing: {err}")
        ...
        population = elites + offspring + temp

        if np.isclose(hall_of_fame[0].fitness.values[0], epsilon) or hall_of_fame[0].fitness.values[0] < epsilon:
            break
